...
describe('TheCatAPI Integration Tests', () => {
    ...
    describe('Images and Votes relationship', () => {
        it('should create vote for image and verify connection', async () => {
            ...
            expect(vote.image).to.exist;
            expect(vote.image?.id).to.equal(imageId);
            expect(vote.image?.url).to.equal(imageUrl);
            ...
            expect(foundVote?.image).to.exist;
            expect(foundVote?.image?.id).to.equal(imageId);
            expect(foundVote?.image?.url).to.equal(imageUrl);
    ...
    describe('Images and Favourites relationship', () => {
        it('should add image to favourites and verify connection', async () => {
            ...
            expect(favourite.image).to.exist;
            expect(favourite.image?.id).to.equal(imageId);
            expect(favourite.image?.url).to.equal(imageUrl);
            ...
            expect(foundFavourite?.image).to.exist;
            expect(foundFavourite?.image?.id).to.equal(imageId);
            expect(foundFavourite?.image?.url).to.equal(imageUrl);
    ...
    describe('All modules together', () => {
        it('should work with same image across all modules', async () => {
            ...
            expect(vote.image?.id).to.equal(imageId);
            expect(favourite.image?.id).to.equal(imageId);
            expect(vote.image?.id).to.equal(favourite.image?.id);
            expect(vote.image?.url).to.equal(imageUrl);
            expect(favourite.image?.url).to.equal(imageUrl);
    ...
    describe('Multiple votes for one image', () => {
        it('should allow multiple votes for same image', async () => {
            ...
            expect(vote1.image).to.exist;
            expect(vote2.image).to.exist;
            expect(vote1.image?.id).to.equal(imageId);
            expect(vote2.image?.id).to.equal(imageId);
            expect(vote1.image?.url).to.equal(imageUrl);
            expect(vote2.image?.url).to.equal(imageUrl);
